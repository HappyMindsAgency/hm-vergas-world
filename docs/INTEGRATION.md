...
- **ADR-15 · Slug pubblici allineati al SEO (orchestratore):** route rinominate per aderire agli slug di `testi-vergasworld.pdf`: `/borgo-cunziria`→`/borgo-della-cunziria`, `/famiglie`→`/visite-in-famiglia`, `/scuole`→`/visite-scolastiche`. Aggiornati i link in `Header.astro` e `index.astro`. Vecchi URL non più validi (no redirect: sito non ancora pubblico).
